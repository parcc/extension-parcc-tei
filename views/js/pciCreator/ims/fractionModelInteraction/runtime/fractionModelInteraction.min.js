...
normalize(name)},cssAPI.load=function(cssId,req,load,config){(useImportLoad?importLoad:linkLoad)(req.toUrl(cssId+".css"),load)},cssAPI}),define("css!fractionModelInteraction/runtime/css/fractionModelInteraction",[],function(){}),define("fractionModelInteraction/runtime/fractionModelInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/scale.raphael","fractionModelInteraction/runtime/libs/pieChart","css!fractionModelInteraction/runtime/css/fractionModelInteraction.css"],function($,qtiCustomInteractionContext,event,_,scaleRaphael){"use strict";
function createCanvas($container,config){config=_.defaults(config,_defaults);var canvasSize=2*(parseInt(config.radius)+parseInt(config.padding)+parseInt(config.outlineThickness)),canvas=scaleRaphael($(".shape-container",$container)[0],canvasSize,canvasSize);return canvas}var _defaults={radius:100,padding:10,outlineThickness:1},fractionModelInteraction={id:-1,getTypeIdentifier:function(){return"fractionModelInteraction"},initialize:function(id,dom,config){event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{};var numerator,denominator,selectedPartitions,_this=this,$container=$(dom);this.setFractionModel=function(num,den){numerator=parseInt(num),
denominator=parseInt(den)},this.getFractionModel=function(){return{numerator:numerator,denominator:denominator}},this.setState=function(stateData){if(_.isString(stateData))""===stateData.trim()||"[]"===stateData.trim()?selectedPartitions=_.range(denominator).map(function(i){return i<numerator}):(selectedPartitions=JSON.parse(stateData),denominator=selectedPartitions.length);else{if(!_.isArray(stateData))throw"invalid format for selectedPartitions";selectedPartitions=stateData,denominator=selectedPartitions.length}},this.getState=function(){return _.clone(selectedPartitions)},this.setFractionModel(this.config.selectedPartitionsInit,this.config.partitionInit);var canvas=createCanvas($container,config);
this.setState(config.selectedPartitions),canvas.pieChart(this.getState(),this.config,$container),$container.on("click.fraction","button.more",function(){if(denominator<_this.config.partitionMax){denominator+=1;var newState=_this.getState();newState.push(!1),_this.setState(newState),$container.trigger("statechange.fraction")}}).on("click.fraction","button.fewer",function(){if(denominator>_this.config.partitionMin){denominator-=1;var newState=_this.getState();newState.pop(),_this.setState(newState),$container.trigger("statechange.fraction")}}).on("click.fraction","button.reset",function(){_this.setFractionModel(_this.config.selectedPartitionsInit,_this.config.partitionInit),
_this.setState(_this.config.selectedPartitions),$container.trigger("reset.fraction")}).on("statechange.fraction reset.fraction",function(event){canvas.clear(),canvas.pieChart(_this.getState(),_this.config,$container),_this.trigger("changepartition",[_this.getState()]),_this.trigger("responseChange",[_this.getResponse()])}).on("select_slice.pieChart unselect_slice.pieChart",function(e,selectedPartitions,totalSelected){numerator=totalSelected,_this.setState(selectedPartitions),_this.trigger("selectedpartition",[selectedPartitions]),_this.trigger("responseChange",[_this.getResponse()])}),this.on("configchange",function(config){_this.config=config,_this.setFractionModel(_this.config.selectedPartitionsInit,_this.config.partitionInit),
_this.setState(_this.config.selectedPartitions),canvas.clear(),canvas=createCanvas($container,_this.config),canvas.pieChart(_this.getState(),_this.config,$container)})},setResponse:function(response){var i,parts,numerator,denominator,state=[],value="0";if(response&&response.base&&response.base.string){for(value=response.base.string,parts=value.split("/"),numerator=parseInt(parts[0]),denominator=parseInt(parts[1]||0),i=0;i<denominator;i++)state.push(i<numerator);this.setState(state),$(this.dom).trigger("statechange.fraction")}},getResponse:function(){var selection=_.values(this.getState()),numerator=_.filter(selection).length,denominator=selection.length;return{
base:{string:numerator+"/"+denominator}}},resetResponse:function(){this.setFractionModel(this.config.selectedPartitionsInit,this.config.partitionInit),this.setState(this.config.selectedPartitions),$(this.dom).trigger("statechange.fraction")},destroy:function(){var $container=$(this.dom);$container.off().empty()},setSerializedState:function(state){state&&_.isArray(state.selection)&&(this.setState(state.selection),$(this.dom).trigger("statechange.fraction"))},getSerializedState:function(){return{selection:_.values(this.getState())}}};qtiCustomInteractionContext.register(fractionModelInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");
s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\n\n.fractionModelInteraction {width:100%;}\n\n.fractionModelInteraction .prompt {position:relative; border : 2px solid #EFEFEF;}\n\n.fractionModelInteraction .likert-label {min-width:90px; color: #444;}\n.fractionModelInteraction .likert-label-min {text-align: right;}\n.fractionModelInteraction .likert-label-max {text-align: left;}\n\n.fractionModelInteraction .shape-title {margin-top: 10px; margin-right : auto; margin-left : auto; width : 50%; text-align : center;}\n.fractionModelInteraction .shape-panel {padding: 10px 10px 20px; background : #EFEFEF;}\n.fractionModelInteraction .shape-container {margin:auto;}\n.fractionModelInteraction .shape-controls {text-align: center;}\n.fractionModelInteraction .shape-controls button {margin: 0 5px; padding: 0 20px;}"),
