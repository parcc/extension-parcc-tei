...
pts.push(pt.getCartesianCoord())}),{points:pts,config:_.cloneDeep(config)}},setState:function(state){if(state.config&&setConfig(state.config),_.each(points,function(point){point.remove()}),points=[],state.points){var i=0,maxPoints=config.maximumPoints;_.each(state.points,function(point){return i<maxPoints&&(addPoint(point.x,point.y,!0),void i++)})}pointsWrapper.disactivate()}};return pointsWrapper}var _defaults={pointColor:"#bb1a2a",pointRadius:10,maximumPoints:1};return{initialize:initialize}}),define("graphLineAndPointInteraction/runtime/wrappers/points",["graphLineAndPointInteraction/runtime/wrappers/setOfPoints"],function(setOfPoints){function initialize(grid,config){
config.max=1;var point=setOfPoints.initialize(grid,config);return point.type="point",point}return{initialize:initialize}}),define("parccTei/portableLib/graphFunction",["taoQtiItem/portableLib/lodash"],function(_){"use strict";function _round(number,decimal){var m=Math.pow(10,parseInt(decimal));return Math.round(number*m)/m}function _isPoint(point){return void 0!==point.x&&void 0!==point.y}function checkPairOfPoints(point1,point2){return _isPoint(point1)&&_isPoint(point2)&&point1.x!==point2.x}function plotCurvedEquation(canvas,equation,config,calc){function jump(){x+=config.precision,pathMove=!0}function getBoundedTop(top){return top<topMin?topMin:top>topMax?topMax:top;
...
point.remove()}),points=[],state.points&&_.each(state.points,function(point){addPoint(point.x,point.y,!0)}),linesWrapper.disactivate()}};return linesWrapper}var _defaults={pointColor:"#bb1a2a",lineColor:"#bb1a2a",lineStyle:"",lineWeight:3,pointRadius:10};return{initialize:initialize}}),define("graphLineAndPointInteraction/runtime/wrappers/segments",["graphLineAndPointInteraction/runtime/wrappers/lines"],function(lines){function initialize(grid,config){config.segment=!0;var segment=lines.initialize(grid,config);return segment.type="segment",segment}return{initialize:initialize}}),define("graphLineAndPointInteraction/runtime/wrappers/solutionSet",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash"],function($,_){
...
if(sheet.href==link.href)return clearInterval(loadInterval),callback()}},10);link.href=url,head.appendChild(link)};return cssAPI.normalize=function(name,normalize){return".css"==name.substr(name.length-4,4)&&(name=name.substr(0,name.length-4)),normalize(name)},cssAPI.load=function(cssId,req,load,config){(useImportLoad?importLoad:linkLoad)(req.toUrl(cssId+".css"),load)},cssAPI}),define("css!graphLineAndPointInteraction/runtime/css/graphLineAndPointInteraction",[],function(){}),define("graphLineAndPointInteraction/runtime/graphLineAndPointInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/scale.raphael","taoQtiItem/portableLib/raphael","parccTei/portableLib/gridFactory","graphLineAndPointInteraction/runtime/wrappers/setOfPoints","graphLineAndPointInteraction/runtime/wrappers/points","graphLineAndPointInteraction/runtime/wrappers/lines","graphLineAndPointInteraction/runtime/wrappers/segments","graphLineAndPointInteraction/runtime/wrappers/solutionSet","css!graphLineAndPointInteraction/runtime/css/graphLineAndPointInteraction"],function($,qtiCustomInteractionContext,event,_,scaleRaphael,Raphael,gridFactory,setPointsWrapper,pointsWrapper,linesWrapper,segmentsWrapper,solutionSetWrapper){
"use strict";function buildGridConfig(rawConfig){return{x:{start:void 0===rawConfig.xMin?-10:parseInt(rawConfig.xMin),end:void 0===rawConfig.xMax?10:parseInt(rawConfig.xMax),unit:20},y:{start:void 0===rawConfig.yMax?10:-1*parseInt(rawConfig.yMax),end:void 0===rawConfig.yMin?-10:-1*parseInt(rawConfig.yMin),unit:20},graphs:void 0===rawConfig.graphs?{}:rawConfig.graphs,padding:20}}function createCanvas($container,config){var padding=2*config.padding,paper=scaleRaphael($(".shape-container",$container)[0],(config.x.end-config.x.start)*config.x.unit+padding,(config.y.end-config.y.start)*config.y.unit+padding);return paper}function getWrapper(type){return _wrappers[type];
}function drawLineStyle(dom,config){var w=57,h=20,lineStylePaper=new Raphael(dom,w,h),line=lineStylePaper.path("M0 "+h/2+"L"+w+" "+h/2);line.attr({stroke:config.lineColor||"#000","stroke-width":config.lineWeight||3,"stroke-dasharray":config.lineStyle||"",opacity:config.opacity||1})}function isValidRecordEntry(entry){return _.isPlainObject(entry)&&_.isString(entry.name)&&entry.base&&entry.base.list&&_.isArray(entry.base.list.point)}function formatResponseElement(name,points){if(_.isString(name),_.isArray(points))return points=_.map(points,function(point){return[point.x,point.y]}),{name:name,base:{list:{point:points}}};throw"invalid arguments"}var _wrappers={setPoints:setPointsWrapper,
points:pointsWrapper,lines:linesWrapper,segments:segmentsWrapper,solutionSet:solutionSetWrapper},responseChangeEvents=["drawn.lines","moved.point","removed.point","added.pointSet","moved.pointSet","unselected.solutionSet","selected.solutionSet"],graphLineAndPointInteraction={id:-1,getTypeIdentifier:function(){return"graphLineAndPointInteraction"},initialize:function(id,dom,config){function initGrid($container,gridConfig){elements={};var grid,paper=createCanvas($container,gridConfig),$canvas=$(paper.canvas);return _.isObject(gridConfig.x)&&_.isObject(gridConfig.y)&&gridConfig.x.start<gridConfig.x.end&&gridConfig.y.start<gridConfig.y.end&&(grid=gridFactory(paper,gridConfig),
grid.clickable(),grid.children.click(function(event){var bnds=event.target.getBoundingClientRect(),wfactor=paper.w/paper.width,fx=grid.getX()+Math.round((event.clientX-bnds.left)/bnds.width*grid.getWidth()*wfactor),fy=grid.getY()+Math.round((event.clientY-bnds.top)/bnds.height*grid.getHeight()*wfactor);$canvas.trigger("click_grid",{x:fx,y:fy})})),grid}function initInteraction(grid,$cont,options){var $templates=$cont.find(".templates"),$controlArea=$cont.find(".shape-controls");if($controlArea.empty(),_.each(options.graphs,function(graphType,typeName){var $template=$templates.find(".template-"+typeName);_.each(graphType.elements,function(elementConfig){var $buttonContainer=$template.children().first().clone(),$button=$buttonContainer.children(".btn"),$arrow=$buttonContainer.children(".triangle"),color=elementConfig.lineColor||elementConfig.pointColor;
$buttonContainer.attr("data-uid",elementConfig.uid),$buttonContainer.data("config",elementConfig),$button.text(elementConfig.label),$button.css({backgroundColor:color}),$arrow.css({borderColor:"transparent transparent transparent "+color}),elementConfig.lineStyleToggle&&"false"!==elementConfig.lineStyleToggle&&$buttonContainer.find(".line-styles").show().find("input[name=line-style]").attr("name","line-style-"+elementConfig.uid).change(function(){elementConfig.lineStyle=$(this).val(),$buttonContainer.data("element").setLineStyle(elementConfig.lineStyle),$button.click()}).each(function(){var $input=$(this),$lineStyle=$input.siblings(".line-style"),style=$input.val();
drawLineStyle($lineStyle[0],{lineStyle:style,lineColor:elementConfig.lineColor,lineWeight:elementConfig.lineWeight}),elementConfig.lineStyle===style&&$input.prop("checked",!0)}),$controlArea.append($buttonContainer);var wrapper=getWrapper(typeName);if(grid){var element=wrapper.initialize(grid,elementConfig);$buttonContainer.data("element",element),element.$buttonContainer=$buttonContainer,elements[elementConfig.uid]=element}})}),$controlArea.on("click",".button-container:not(.deactivated)",function(){activate($(this).data("element"))}).on("mouseenter",".button-container:not(.deactivated)",function(){var element=$(this).data("element");element&&element.highlightOn();
}).on("mouseleave",".button-container:not(.deactivated)",function(){var element=$(this).data("element");element&&!element.isActive()&&element.highlightOff()}),$container.on(responseChangeEvents.join(" "),_.debounce(function(){self.trigger("responseChange",[self.getResponse()])},100)),grid){var $solutionSet=$controlArea.find(".graph-solutionSet");$(grid.getCanvas().canvas).on("drawn.lines removed.lines",function(){var drawnLineExists=!1;_.each(_.where(elements,{type:"line"}),function(line){var drawnLine=line.getLine();if(drawnLine)return drawnLineExists=!0,!1}),drawnLineExists?$solutionSet.removeClass("deactivated"):$solutionSet.addClass("deactivated")})}}function activate(element){
graph&&graph.disactivate(),element.$buttonContainer.addClass("activated"),element.$buttonContainer.siblings(".button-container").removeClass("activated"),element.activate(elements),graph=element}function getElement(uid){return elements[uid]}function getState(){var state={activeGraph:graph?graph.getId():"",elements:{}};return _.each(elements,function(element){state.elements[element.getId()]=element.getState()}),state}function setState(state,ignoreConfig){if(_.forIn(state.elements,function(state,uid){var element=getElement(uid);element&&(ignoreConfig&&delete state.config,element.setState(state))}),state.activeGraph){var currentActive=getElement(state.activeGraph);
currentActive&&activate(currentActive)}}this.id=id,this.dom=dom,this.config=buildGridConfig(config||{}),event.addEventMgr(this);var grid,graph,elements,$container=$(dom),self=this;this.getRawResponse=function(){var response=[],states=[];return _.each(elements,function(element,id){var res={id:id,type:element.type};"solutionSet"===element.type?res.selections=element.getState().selections:res.points=element.getState().points,response.push(res),states.push(element.getState())}),response},this.setRawResponse=function(response){_.each(response,function(res){var state={},element=elements[res.id];element&&("solutionSet"===res.type?(element.createSolutionSet(elements),
state.selections=res.selections):state.points=res.points,element.setState(state))})},grid=initGrid($container,this.config),initInteraction(grid,$container,this.config),this.on("configchange",function(newConfig){var state=getState();self.config=newConfig?buildGridConfig(newConfig):self.config,grid=initGrid($container,self.config),initInteraction(grid,$container,self.config),setState(state,!0)}),this.on("gridchange",function(newConfig){self.config=newConfig?buildGridConfig(newConfig):self.config,grid=initGrid($container,self.config),initInteraction(grid,$container,self.config)})},setResponse:function(response){var rawResponse=[],solutionSetSelections=[],solutionSetId="";
response&&_.isArray(response.record)&&(_.each(response.record,function(entry){var points,id;isValidRecordEntry(entry)&&(id=entry.name,points=_.map(entry.base.list.point,function(point){return{x:point[0],y:point[1]}}),id.match(/^solutionSet/)?(solutionSetId=id,solutionSetSelections.push(points)):rawResponse.push({id:id,points:points}))}),solutionSetId&&solutionSetSelections.length&&rawResponse.push({id:solutionSetId,type:"solutionSet",selections:solutionSetSelections}),this.setRawResponse(rawResponse))},getResponse:function(){var rawResponse=this.getRawResponse(),response={record:[]};return _.each(rawResponse,function(element){"solutionSet"===element.type?_.each(element.selections,function(selection){
response.record.push(formatResponseElement(element.id,selection))}):response.record.push(formatResponseElement(element.id,element.points))}),response},resetResponse:function(){},destroy:function(){var $container=$(this.dom);$container.off().empty()},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){return this.getResponse()}};qtiCustomInteractionContext.register(graphLineAndPointInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\n\n.pointAndLineFunctionInteraction {\n    width: 100%;\n    margin: 20px 0;\n    padding: 20px;\n    border : 6px solid #f5f3f2;\n}\n\n.pointAndLineFunctionInteraction .shape-controls .clearfix {\n    overflow: auto;\n}\n\n.pointAndLineFunctionInteraction .prompt {margin-bottom:20px;}\n.pointAndLineFunctionInteraction .shape-panel {position:relative; display: inline-block; width: 100%;}\n\n.pointAndLineFunctionInteraction .shape-panel .templates {display:none;}\n\n.pointAndLineFunctionInteraction .shape-panel .shape-container {float:left;}\n\n.pointAndLineFunctionInteraction .shape-panel .shape-controls {float:left; margin-bottom:20px;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container {width:200px; margin:3px 0; display:block; position:relative; padding: 3px;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .btn {width:150px; float:left;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .triangle{\n    width: 0;\n    height: 0;\n    border-style: solid;\n    border-width: 16px 0 16px 27.7px;\n    border-color: transparent transparent transparent #266d9c;\n    float:left;\n    top:2px;\n    position: relative;\n    visibility: hidden;\n}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container.activated .triangle{\n    visibility: visible;\n}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .btn:hover + .triangle {opacity:.85;}\n\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .line-styles {display:none; margin-top:30px; padding:10px;width:150px;border: solid 3px #EFEFEF;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .line-style {display:inline-block; margin-left:3px;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container svg {top:5px;}\n\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container.deactivated .btn {background : #AFB4BB !important; text-shadow:none;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container.deactivated .btn:hover {opacity:1; cursor:default;}"),
