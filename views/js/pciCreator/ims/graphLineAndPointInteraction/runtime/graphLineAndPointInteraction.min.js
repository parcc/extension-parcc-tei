...
}var graphLineAndPointInteraction,_wrappers={setPoints:setPointsWrapper,points:pointsWrapper,lines:linesWrapper,segments:segmentsWrapper,solutionSet:solutionSetWrapper},responseChangeEvents=["drawn.lines","moved.point","removed.point","added.pointSet","moved.pointSet","unselected.solutionSet","selected.solutionSet"],_typeIdentifier="graphLineAndPointInteraction";graphLineAndPointInteraction={typeIdentifier:_typeIdentifier,getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState());
},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var rawResponse=this.getRawResponse(),response={record:[]};return _.each(rawResponse,function(element){"solutionSet"===element.type?_.each(element.selections,function(selection){response.record.push(formatResponseElement(element.id,selection))}):response.record.push(formatResponseElement(element.id,element.points))}),response},destroy:function(){var $container=$(this.dom);$container.off().empty()},id:-1,getTypeIdentifier:function(){return _typeIdentifier},initialize:function(id,dom,config){function initGrid($container,gridConfig){var grid,paper=createCanvas($container,gridConfig),$canvas=$(paper.canvas);
return elements={},_.isObject(gridConfig.x)&&_.isObject(gridConfig.y)&&gridConfig.x.start<gridConfig.x.end&&gridConfig.y.start<gridConfig.y.end&&(grid=gridFactory(paper,gridConfig),grid.clickable(),grid.children.click(function(event){var bnds=event.target.getBoundingClientRect(),wfactor=paper.w/paper.width,fx=grid.getX()+Math.round((event.clientX-bnds.left)/bnds.width*grid.getWidth()*wfactor),fy=grid.getY()+Math.round((event.clientY-bnds.top)/bnds.height*grid.getHeight()*wfactor);$canvas.trigger("click_grid",{x:fx,y:fy})})),grid}function initInteraction(grid,$cont,options){var $templates=$cont.find(".templates"),$controlArea=$cont.find(".shape-controls");if($controlArea.empty(),
_.each(options.graphs,function(graphType,typeName){var $template=$templates.find(".template-"+typeName);_.each(graphType.elements,function(elementConfig){var $buttonContainer=$template.children().first().clone(),$button=$buttonContainer.children(".btn"),$arrow=$buttonContainer.children(".triangle"),color=elementConfig.lineColor||elementConfig.pointColor;$buttonContainer.attr("data-uid",elementConfig.uid),$buttonContainer.data("config",elementConfig),$button.text(elementConfig.label),$button.css({backgroundColor:color}),$arrow.css({borderColor:"transparent transparent transparent "+color}),elementConfig.lineStyleToggle&&"false"!==elementConfig.lineStyleToggle&&$buttonContainer.find(".line-styles").show().find("input[name=line-style]").attr("name","line-style-"+elementConfig.uid).change(function(){
elementConfig.lineStyle=$(this).val(),$buttonContainer.data("element").setLineStyle(elementConfig.lineStyle),$button.click()}).each(function(){var $input=$(this),$lineStyle=$input.siblings(".line-style"),style=$input.val();drawLineStyle($lineStyle[0],{lineStyle:style,lineColor:elementConfig.lineColor,lineWeight:elementConfig.lineWeight}),elementConfig.lineStyle===style&&$input.prop("checked",!0)}),$controlArea.append($buttonContainer);var wrapper=getWrapper(typeName);if(grid){var element=wrapper.initialize(grid,elementConfig);$buttonContainer.data("element",element),element.$buttonContainer=$buttonContainer,elements[elementConfig.uid]=element}})}),$controlArea.on("click",".button-container:not(.deactivated)",function(){
activate($(this).data("element"))}).on("mouseenter",".button-container:not(.deactivated)",function(){var element=$(this).data("element");element&&element.highlightOn()}).on("mouseleave",".button-container:not(.deactivated)",function(){var element=$(this).data("element");element&&!element.isActive()&&element.highlightOff()}),$container.on(responseChangeEvents.join(" "),_.debounce(function(){self.trigger("responseChange",[self.getResponse()])},100)),grid){var $solutionSet=$controlArea.find(".graph-solutionSet");$(grid.getCanvas().canvas).on("drawn.lines removed.lines",function(){var drawnLineExists=!1;_.each(_.where(elements,{type:"line"}),function(line){var drawnLine=line.getLine();
if(drawnLine)return drawnLineExists=!0,!1}),drawnLineExists?$solutionSet.removeClass("deactivated"):$solutionSet.addClass("deactivated")})}}function activate(element){graph&&graph.disactivate(),element.$buttonContainer.addClass("activated"),element.$buttonContainer.siblings(".button-container").removeClass("activated"),element.activate(elements),graph=element}function getElement(uid){return elements[uid]}function getState(){var state={activeGraph:graph?graph.getId():"",elements:{}};return _.each(elements,function(element){state.elements[element.getId()]=element.getState()}),state}function setState(state,ignoreConfig){if(_.forIn(state.elements,function(state,uid){
var element=getElement(uid);element&&(ignoreConfig&&delete state.config,element.setState(state))}),state.activeGraph){var currentActive=getElement(state.activeGraph);currentActive&&activate(currentActive)}}var grid,graph,elements,$container=$(dom),self=this;this.id=id,this.dom=dom,this.config=buildGridConfig(config||{}),event.addEventMgr(this),this.getRawResponse=function(){var response=[],states=[];return _.each(elements,function(element,id){var res={id:id,type:element.type};"solutionSet"===element.type?res.selections=element.getState().selections:res.points=element.getState().points,response.push(res),states.push(element.getState())}),response},this.setRawResponse=function(response){
_.each(response,function(res){var state={},element=elements[res.id];element&&("solutionSet"===res.type?(element.createSolutionSet(elements),state.selections=res.selections):state.points=res.points,element.setState(state))})},grid=initGrid($container,this.config),initInteraction(grid,$container,this.config),this.on("configchange",function(newConfig){var state=getState();self.config=newConfig?buildGridConfig(newConfig):self.config,grid=initGrid($container,self.config),initInteraction(grid,$container,self.config),setState(state,!0)}),this.on("gridchange",function(newConfig){self.config=newConfig?buildGridConfig(newConfig):self.config,grid=initGrid($container,self.config),
initInteraction(grid,$container,self.config)})},setResponse:function(response){var rawResponse=[],solutionSetSelections=[],solutionSetId="";response&&_.isArray(response.record)&&(_.each(response.record,function(entry){var points,id;isValidRecordEntry(entry)&&(id=entry.name,points=_.map(entry.base.list.point,function(point){return{x:point[0],y:point[1]}}),id.match(/^solutionSet/)?(solutionSetId=id,solutionSetSelections.push(points)):rawResponse.push({id:id,points:points}))}),solutionSetId&&solutionSetSelections.length&&rawResponse.push({id:solutionSetId,type:"solutionSet",selections:solutionSetSelections}),this.setRawResponse(rawResponse))},resetResponse:function(){},
