...
return compile(input,options,hb)},hb.precompile=function(input,options){return precompile(input,options,hb)},hb.AST=AST,hb.Compiler=Compiler,hb.JavaScriptCompiler=JavaScriptCompiler,hb.Parser=Parser,hb.parse=parse,hb};return Handlebars=create(),Handlebars.create=create,__exports__=Handlebars}(__module1__,__module7__,__module8__,__module10__,__module11__);return __module0__}();return Handlebars}),define("taoQtiItem/portableLib/OAT/waitForMedia",["taoQtiItem/portableLib/jquery_2_1_1"],function($){"use strict";var _ns=".wait";$.fn.waitForMedia=function(allLoadedCallback){function allLoaded($container){$container.trigger("all-loaded"+_ns),"function"==typeof allLoadedCallback&&allLoadedCallback.call($container[0]);
}return this.each(function(){var $container=$(this),$img=$container.find("img"),count=$img.length,loaded=0,imageLoaded=function(){$(this).trigger("loaded"+_ns).off("load"+_ns).off("error"+_ns),loaded++,loaded===count&&allLoaded($container)};return 0===count?void allLoaded($container):void $img.each(function(){this.complete?imageLoaded.call(this):$(this).on("load"+_ns+" error"+_ns,imageLoaded)})})}}),define("parccTei/pciCreator/ims/multiTabbedExhibit/runtime/lib/sly",["taoQtiItem/portableLib/jquery_2_1_1"],function(jQuery){!function($,w,undefined){"use strict";function Sly(frame,options,callbackMap){function load(){if(self.initialized){var lastItemsCount=0,lastPagesCount=pages.length;
if(pos.old=$.extend({},pos),frameSize=parallax?0:$frame[o.horizontal?"width":"height"](),sbSize=$sb[o.horizontal?"width":"height"](),slideeSize=parallax?frame:$slidee[o.horizontal?"outerWidth":"outerHeight"](),pages.length=0,pos.start=0,pos.end=max(slideeSize-frameSize,0),itemNav){lastItemsCount=items.length,$items=$slidee.children(o.itemSelector),items.length=0;var lastItem,paddingStart=getPx($slidee,o.horizontal?"paddingLeft":"paddingTop"),paddingEnd=getPx($slidee,o.horizontal?"paddingRight":"paddingBottom"),borderBox="border-box"===$($items).css("boxSizing"),areFloated="none"!==$items.css("float"),ignoredMargin=0,lastItemIndex=$items.length-1;slideeSize=0,
$items.each(function(i,element){var $item=$(element),itemSize=$item[o.horizontal?"outerWidth":"outerHeight"](),itemMarginStart=getPx($item,o.horizontal?"marginLeft":"marginTop"),itemMarginEnd=getPx($item,o.horizontal?"marginRight":"marginBottom"),itemSizeFull=itemSize+itemMarginStart+itemMarginEnd,singleSpaced=!itemMarginStart||!itemMarginEnd,item={};item.el=element,item.size=singleSpaced?itemSize:itemSizeFull,item.half=item.size/2,item.start=slideeSize+(singleSpaced?itemMarginStart:0),item.center=item.start-round(frameSize/2-item.size/2),item.end=item.start-frameSize+item.size,i||(slideeSize+=paddingStart),slideeSize+=itemSizeFull,o.horizontal||areFloated||itemMarginEnd&&itemMarginStart&&i>0&&(slideeSize-=min(itemMarginStart,itemMarginEnd)),
i===lastItemIndex&&(item.end+=paddingEnd,slideeSize+=paddingEnd,ignoredMargin=singleSpaced?itemMarginEnd:0),items.push(item),lastItem=item}),$slidee[0].style[o.horizontal?"width":"height"]=(borderBox?slideeSize:slideeSize-paddingStart-paddingEnd)+"px",slideeSize-=ignoredMargin,items.length?(pos.start=items[0][forceCenteredNav?"center":"start"],pos.end=forceCenteredNav?lastItem.center:frameSize<slideeSize?lastItem.end:pos.start):pos.start=pos.end=0}if(pos.center=round(pos.end/2+pos.start/2),updateRelatives(),$handle.length&&sbSize>0&&(o.dynamicHandle?(handleSize=pos.start===pos.end?sbSize:round(sbSize*frameSize/slideeSize),handleSize=within(handleSize,o.minHandleSize,sbSize),
$handle[0].style[o.horizontal?"width":"height"]=handleSize+"px"):handleSize=$handle[o.horizontal?"outerWidth":"outerHeight"](),hPos.end=sbSize-handleSize,renderID||syncScrollbar()),!parallax&&frameSize>0){var tempPagePos=pos.start,pagesHtml="";if(itemNav)$.each(items,function(i,item){forceCenteredNav?pages.push(item.center):item.start+item.size>tempPagePos&&tempPagePos<=pos.end&&(tempPagePos=item.start,pages.push(tempPagePos),tempPagePos+=frameSize,tempPagePos>pos.end&&tempPagePos<pos.end+frameSize&&pages.push(pos.end))});else for(;tempPagePos-frameSize<pos.end;)pages.push(tempPagePos),tempPagePos+=frameSize;if($pb[0]&&lastPagesCount!==pages.length){for(var i=0;i<pages.length;i++)pagesHtml+=o.pageBuilder.call(self,i);
$pages=$pb.html(pagesHtml).children(),$pages.eq(rel.activePage).addClass(o.activeClass)}}if(rel.slideeSize=slideeSize,rel.frameSize=frameSize,rel.sbSize=sbSize,rel.handleSize=handleSize,itemNav){self.initialized?(rel.activeItem>=items.length||0===lastItemsCount&&items.length>0)&&activate(rel.activeItem>=items.length?items.length-1:0,!lastItemsCount):(activate(o.startAt),self[centeredNav?"toCenter":"toStart"](o.startAt));var activeItem=items[rel.activeItem];slideTo(centeredNav&&activeItem?activeItem.center:within(pos.dest,pos.start,pos.end))}else self.initialized?slideTo(within(pos.dest,pos.start,pos.end)):slideTo(o.startAt,1);trigger("load")}}function slideTo(newPos,immediate,dontAlign){
if(itemNav&&dragging.released&&!dontAlign){var tempRel=getRelatives(newPos),isNotBordering=newPos>pos.start&&newPos<pos.end;centeredNav?(isNotBordering&&(newPos=items[tempRel.centerItem].center),forceCenteredNav&&o.activateMiddle&&activate(tempRel.centerItem)):isNotBordering&&(newPos=items[tempRel.firstItem].start)}dragging.init&&dragging.slidee&&o.elasticBounds?newPos>pos.end?newPos=pos.end+(newPos-pos.end)/6:newPos<pos.start&&(newPos=pos.start+(newPos-pos.start)/6):newPos=within(newPos,pos.start,pos.end),animation.start=+new Date,animation.time=0,animation.from=pos.cur,animation.to=newPos,animation.delta=newPos-pos.cur,animation.tweesing=dragging.tweese||dragging.init&&!dragging.slidee,
animation.immediate=!animation.tweesing&&(immediate||dragging.init&&dragging.slidee||!o.speed),dragging.tweese=0,newPos!==pos.dest&&(pos.dest=newPos,trigger("change"),renderID||render()),resetCycle(),updateRelatives(),updateButtonsState(),syncPagesbar()}function render(){if(self.initialized){if(!renderID)return renderID=rAF(render),void(dragging.released&&trigger("moveStart"));animation.immediate?pos.cur=animation.to:animation.tweesing?(animation.tweeseDelta=animation.to-pos.cur,abs(animation.tweeseDelta)<.1?pos.cur=animation.to:pos.cur+=animation.tweeseDelta*(dragging.released?o.swingSpeed:o.syncSpeed)):(animation.time=min(+new Date-animation.start,o.speed),
pos.cur=animation.from+animation.delta*jQuery.easing[o.easing](animation.time/o.speed,animation.time,0,1,o.speed)),animation.to===pos.cur?(pos.cur=animation.to,dragging.tweese=renderID=0):renderID=rAF(render),trigger("move"),parallax||(transform?$slidee[0].style[transform]=gpuAcceleration+(o.horizontal?"translateX":"translateY")+"("+-pos.cur+"px)":$slidee[0].style[o.horizontal?"left":"top"]=-round(pos.cur)+"px"),!renderID&&dragging.released&&trigger("moveEnd"),syncScrollbar()}}function syncScrollbar(){$handle.length&&(hPos.cur=pos.start===pos.end?0:((dragging.init&&!dragging.slidee?pos.dest:pos.cur)-pos.start)/(pos.end-pos.start)*hPos.end,hPos.cur=within(round(hPos.cur),hPos.start,hPos.end),
last.hPos!==hPos.cur&&(last.hPos=hPos.cur,transform?$handle[0].style[transform]=gpuAcceleration+(o.horizontal?"translateX":"translateY")+"("+hPos.cur+"px)":$handle[0].style[o.horizontal?"left":"top"]=hPos.cur+"px"))}function syncPagesbar(){$pages[0]&&last.page!==rel.activePage&&(last.page=rel.activePage,$pages.removeClass(o.activeClass).eq(rel.activePage).addClass(o.activeClass),trigger("activePage",last.page))}function moveLoop(){move.speed&&pos.cur!==(move.speed>0?pos.end:pos.start)||self.stop(),continuousID=dragging.init?rAF(moveLoop):0,move.now=+new Date,move.pos=pos.cur+(move.now-move.lastTime)/1e3*move.speed,slideTo(dragging.init?move.pos:round(move.pos)),
dragging.init||pos.cur!==pos.dest||trigger("moveEnd"),move.lastTime=move.now}function to(location,item,immediate){if("boolean"===type(item)&&(immediate=item,item=undefined),item===undefined)slideTo(pos[location],immediate);else{if(centeredNav&&"center"!==location)return;var itemPos=self.getPos(item);itemPos&&slideTo(itemPos[location],immediate,!centeredNav)}}function getIndex(item){return null!=item?isNumber(item)?item>=0&&item<items.length?item:-1:$items.index(item):-1}function getRelativeIndex(item){return getIndex(isNumber(item)&&item<0?item+items.length:item)}function activate(item,force){var index=getIndex(item);return!(!itemNav||index<0)&&((last.active!==index||force)&&($items.eq(rel.activeItem).removeClass(o.activeClass),
...
i===il-1||slideePos<=items[i].end+items[i].half){last=i;break}relatives.firstItem=isNumber(first)?first:0,relatives.centerItem=isNumber(center)?center:relatives.firstItem,relatives.lastItem=isNumber(last)?last:relatives.centerItem}return relatives}function updateRelatives(newPos){$.extend(rel,getRelatives(newPos))}function updateButtonsState(){var isStart=pos.dest<=pos.start,isEnd=pos.dest>=pos.end,slideePosState=isStart?1:isEnd?2:3;if(last.slideePosState!==slideePosState&&(last.slideePosState=slideePosState,$prevPageButton.is("button,input")&&$prevPageButton.prop("disabled",isStart),$nextPageButton.is("button,input")&&$nextPageButton.prop("disabled",isEnd),$prevPageButton.add($backwardButton)[isStart?"addClass":"removeClass"](o.disabledClass),
$nextPageButton.add($forwardButton)[isEnd?"addClass":"removeClass"](o.disabledClass)),last.fwdbwdState!==slideePosState&&dragging.released&&(last.fwdbwdState=slideePosState,$backwardButton.is("button,input")&&$backwardButton.prop("disabled",isStart),$forwardButton.is("button,input")&&$forwardButton.prop("disabled",isEnd)),itemNav){var isFirst=0===rel.activeItem,isLast=rel.activeItem>=items.length-1,itemsButtonState=isFirst?1:isLast?2:3;last.itemsButtonState!==itemsButtonState&&(last.itemsButtonState=itemsButtonState,$prevButton.is("button,input")&&$prevButton.prop("disabled",isFirst),$nextButton.is("button,input")&&$nextButton.prop("disabled",isLast),$prevButton[isFirst?"addClass":"removeClass"](o.disabledClass),
$nextButton[isLast?"addClass":"removeClass"](o.disabledClass))}}function moveItem(item,position,after){if(item=getRelativeIndex(item),position=getRelativeIndex(position),item>-1&&position>-1&&item!==position&&(!after||position!==item-1)&&(after||position!==item+1)){$items.eq(item)[after?"insertAfter":"insertBefore"](items[position].el);var shiftStart=item<position?item:after?position:position-1,shiftEnd=item>position?item:after?position+1:position,shiftsUp=item>position;item===rel.activeItem?last.active=rel.activeItem=after?shiftsUp?position+1:position:shiftsUp?position:position-1:rel.activeItem>shiftStart&&rel.activeItem<shiftEnd&&(last.active=rel.activeItem+=shiftsUp?1:-1),
load()}}function callbackIndex(name,fn){for(var i=0,l=callbacks[name].length;i<l;i++)if(callbacks[name][i]===fn)return i;return-1}function resetCycle(){dragging.released&&!self.isPaused&&self.resume()}function handleToSlidee(handlePos){return round(within(handlePos,hPos.start,hPos.end)/hPos.end*(pos.end-pos.start))+pos.start}function draggingHistoryTick(){dragging.history[0]=dragging.history[1],dragging.history[1]=dragging.history[2],dragging.history[2]=dragging.history[3],dragging.history[3]=dragging.delta}function continuousInit(source){dragging.released=0,dragging.source=source,dragging.slidee="slidee"===source}function dragInit(event){var isTouch="touchstart"===event.type,source=event.data.source,isSlidee="slidee"===source;
dragging.init||!isTouch&&isInteractive(event.target)||("handle"!==source||o.dragHandle&&hPos.start!==hPos.end)&&(isSlidee&&!(isTouch?o.touchDragging:o.mouseDragging&&event.which<2)||(isTouch||stopDefault(event),continuousInit(source),dragging.init=0,dragging.$source=$(event.target),dragging.touch=isTouch,dragging.pointer=isTouch?event.originalEvent.touches[0]:event,dragging.initX=dragging.pointer.pageX,dragging.initY=dragging.pointer.pageY,dragging.initPos=isSlidee?pos.cur:hPos.cur,dragging.start=+new Date,dragging.time=0,dragging.path=0,dragging.delta=0,dragging.locked=0,dragging.history=[0,0,0,0],dragging.pathToLock=isSlidee?isTouch?30:10:0,$doc.on(isTouch?dragTouchEvents:dragMouseEvents,dragHandler),
self.pause(1),(isSlidee?$slidee:$handle).addClass(o.draggedClass),trigger("moveStart"),isSlidee&&(historyID=setInterval(draggingHistoryTick,10))))}function dragHandler(event){if(dragging.released="mouseup"===event.type||"touchend"===event.type,dragging.pointer=dragging.touch?event.originalEvent[dragging.released?"changedTouches":"touches"][0]:event,dragging.pathX=dragging.pointer.pageX-dragging.initX,dragging.pathY=dragging.pointer.pageY-dragging.initY,dragging.path=sqrt(pow(dragging.pathX,2)+pow(dragging.pathY,2)),dragging.delta=o.horizontal?dragging.pathX:dragging.pathY,!dragging.init){if(!(o.horizontal?abs(dragging.pathX)>abs(dragging.pathY):abs(dragging.pathX)<abs(dragging.pathY)))return dragEnd();
dragging.init=1}stopDefault(event),!dragging.locked&&dragging.path>dragging.pathToLock&&dragging.slidee&&(dragging.locked=1,dragging.$source.on(clickEvent,disableOneEvent)),dragging.released&&(dragEnd(),o.releaseSwing&&dragging.slidee&&(dragging.swing=(dragging.delta-dragging.history[0])/40*300,dragging.delta+=dragging.swing,dragging.tweese=abs(dragging.swing)>10)),slideTo(dragging.slidee?round(dragging.initPos-dragging.delta):handleToSlidee(dragging.initPos+dragging.delta))}function dragEnd(){clearInterval(historyID),$doc.off(dragging.touch?dragTouchEvents:dragMouseEvents,dragHandler),(dragging.slidee?$slidee:$handle).removeClass(o.draggedClass),setTimeout(function(){
dragging.$source.off(clickEvent,disableOneEvent)}),self.resume(1),pos.cur===pos.dest&&dragging.init&&trigger("moveEnd"),dragging.init=0}function isInteractive(element){return~$.inArray(element.nodeName,interactiveElements)||$(element).is(o.interactive)}function movementReleaseHandler(){self.stop(),$doc.off("mouseup",movementReleaseHandler)}function buttonsHandler(event){switch(stopDefault(event),this){case $forwardButton[0]:case $backwardButton[0]:self.moveBy($forwardButton.is(this)?o.moveBy:-o.moveBy),$doc.on("mouseup",movementReleaseHandler);break;case $prevButton[0]:self.prev();break;case $nextButton[0]:self.next();break;case $prevPageButton[0]:self.prevPage();
break;case $nextPageButton[0]:self.nextPage()}}function normalizeWheelDelta(event){return scrolling.curDelta=(o.horizontal?event.deltaY||event.deltaX:event.deltaY)||-event.wheelDelta,scrolling.curDelta/=1===event.deltaMode?3:100,itemNav?(time=+new Date,scrolling.last<time-scrolling.resetTime&&(scrolling.delta=0),scrolling.last=time,scrolling.delta+=scrolling.curDelta,abs(scrolling.delta)<1?scrolling.finalDelta=0:(scrolling.finalDelta=round(scrolling.delta/1),scrolling.delta%=1),scrolling.finalDelta):scrolling.curDelta}function scrollHandler(event){var time=+new Date;if(lastGlobalWheel+300>time)return void(lastGlobalWheel=time);if(o.scrollBy&&pos.start!==pos.end){
var delta=normalizeWheelDelta(event.originalEvent);(o.scrollTrap||delta>0&&pos.dest<pos.end||delta<0&&pos.dest>pos.start)&&stopDefault(event,1),self.slideBy(o.scrollBy*delta)}}function scrollbarHandler(event){o.clickBar&&event.target===$sb[0]&&(stopDefault(event),slideTo(handleToSlidee((o.horizontal?event.pageX-$sb.offset().left:event.pageY-$sb.offset().top)-handleSize/2)))}function keyboardHandler(event){if(o.keyboardNavBy)switch(event.which){case o.horizontal?37:38:stopDefault(event),self["pages"===o.keyboardNavBy?"prevPage":"prev"]();break;case o.horizontal?39:40:stopDefault(event),self["pages"===o.keyboardNavBy?"nextPage":"next"]()}}function activateHandler(event){
return isInteractive(this)?void event.stopPropagation():void(this.parentNode===$slidee[0]&&self.activate(this))}function activatePageHandler(){this.parentNode===$pb[0]&&self.activatePage($pages.index(this))}function pauseOnHoverHandler(event){o.pauseOnHover&&self["mouseenter"===event.type?"pause":"resume"](2)}function trigger(name,arg1){if(callbacks[name]){for(l=callbacks[name].length,tmpArray.length=0,i=0;i<l;i++)tmpArray.push(callbacks[name][i]);for(i=0;i<l;i++)tmpArray[i].call(self,name,arg1)}}var i,l,o=$.extend({},Sly.defaults,options),self=this,parallax=isNumber(frame),$frame=$(frame),$slidee=$frame.children().eq(0),frameSize=0,slideeSize=0,pos={start:0,
center:0,end:0,cur:0,dest:0},$sb=$(o.scrollBar).eq(0),$handle=$sb.children().eq(0),sbSize=0,handleSize=0,hPos={start:0,end:0,cur:0},$pb=$(o.pagesBar),$pages=0,pages=[],$items=0,items=[],rel={firstItem:0,lastItem:0,centerItem:0,activeItem:-1,activePage:0},frameStyles=new StyleRestorer($frame[0]),slideeStyles=new StyleRestorer($slidee[0]),sbStyles=new StyleRestorer($sb[0]),handleStyles=new StyleRestorer($handle[0]),basicNav="basic"===o.itemNav,forceCenteredNav="forceCentered"===o.itemNav,centeredNav="centered"===o.itemNav||forceCenteredNav,itemNav=!parallax&&(basicNav||centeredNav||forceCenteredNav),$scrollSource=o.scrollSource?$(o.scrollSource):$frame,$dragSource=o.dragSource?$(o.dragSource):$frame,$forwardButton=$(o.forward),$backwardButton=$(o.backward),$prevButton=$(o.prev),$nextButton=$(o.next),$prevPageButton=$(o.prevPage),$nextPageButton=$(o.nextPage),callbacks={},last={},animation={},move={},dragging={
released:1},scrolling={last:0,delta:0,resetTime:200},renderID=0,historyID=0,cycleID=0,continuousID=0;parallax||(frame=$frame[0]),self.initialized=0,self.frame=frame,self.slidee=$slidee[0],self.pos=pos,self.rel=rel,self.items=items,self.pages=pages,self.isPaused=0,self.options=o,self.dragging=dragging,self.reload=load,self.getPos=function(item){if(itemNav){var index=getIndex(item);return index!==-1&&items[index]}var $item=$slidee.find(item).eq(0);if($item[0]){var offset=o.horizontal?$item.offset().left-$slidee.offset().left:$item.offset().top-$slidee.offset().top,size=$item[o.horizontal?"outerWidth":"outerHeight"]();return{start:offset,center:offset-frameSize/2+size/2,
end:offset-frameSize+size,size:size}}return!1},self.moveBy=function(speed){move.speed=speed,!dragging.init&&move.speed&&pos.cur!==(move.speed>0?pos.end:pos.start)&&(move.lastTime=+new Date,move.startPos=pos.cur,continuousInit("button"),dragging.init=1,trigger("moveStart"),cAF(continuousID),moveLoop())},self.stop=function(){"button"===dragging.source&&(dragging.init=0,dragging.released=1)},self.prev=function(){self.activate(rel.activeItem-1)},self.next=function(){self.activate(rel.activeItem+1)},self.prevPage=function(){self.activatePage(rel.activePage-1)},self.nextPage=function(){self.activatePage(rel.activePage+1)},self.slideBy=function(delta,immediate){delta&&(itemNav?self[centeredNav?"toCenter":"toStart"](within((centeredNav?rel.centerItem:rel.firstItem)+o.scrollBy*delta,0,items.length)):slideTo(pos.dest+delta,immediate));
},self.slideTo=function(pos,immediate){slideTo(pos,immediate)},self.toStart=function(item,immediate){to("start",item,immediate)},self.toEnd=function(item,immediate){to("end",item,immediate)},self.toCenter=function(item,immediate){to("center",item,immediate)},self.getIndex=getIndex,self.activate=function(item,immediate){var index=activate(item);o.smart&&index!==!1&&(centeredNav?self.toCenter(index,immediate):index>=rel.lastItem?self.toStart(index,immediate):index<=rel.firstItem?self.toEnd(index,immediate):resetCycle())},self.activatePage=function(index,immediate){isNumber(index)&&slideTo(pages[within(index,0,pages.length-1)],immediate)},self.resume=function(priority){
!o.cycleBy||!o.cycleInterval||"items"===o.cycleBy&&!items[0]||priority<self.isPaused||(self.isPaused=0,cycleID?cycleID=clearTimeout(cycleID):trigger("resume"),cycleID=setTimeout(function(){switch(trigger("cycle"),o.cycleBy){case"items":self.activate(rel.activeItem>=items.length-1?0:rel.activeItem+1);break;case"pages":self.activatePage(rel.activePage>=pages.length-1?0:rel.activePage+1)}},o.cycleInterval))},self.pause=function(priority){priority<self.isPaused||(self.isPaused=priority||100,cycleID&&(cycleID=clearTimeout(cycleID),trigger("pause")))},self.toggle=function(){self[cycleID?"pause":"resume"]()},self.set=function(name,value){$.isPlainObject(name)?$.extend(o,name):o.hasOwnProperty(name)&&(o[name]=value);
},self.add=function(element,index){var $element=$(element);itemNav?(null==index||!items[0]||index>=items.length?$element.appendTo($slidee):items.length&&$element.insertBefore(items[index].el),index<=rel.activeItem&&(last.active=rel.activeItem+=$element.length)):$slidee.append($element),load()},self.remove=function(element){if(itemNav){var index=getRelativeIndex(element);if(index>-1){$items.eq(index).remove();var reactivate=index===rel.activeItem;index<rel.activeItem&&(last.active=--rel.activeItem),load(),reactivate&&(last.active=null,self.activate(rel.activeItem))}}else $(element).remove(),load()},self.moveAfter=function(item,position){moveItem(item,position,1);
},self.moveBefore=function(item,position){moveItem(item,position)},self.on=function(name,fn){if("object"===type(name))for(var key in name)name.hasOwnProperty(key)&&self.on(key,name[key]);else if("function"===type(fn))for(var names=name.split(" "),n=0,nl=names.length;n<nl;n++)callbacks[names[n]]=callbacks[names[n]]||[],callbackIndex(names[n],fn)===-1&&callbacks[names[n]].push(fn);else if("array"===type(fn))for(var f=0,fl=fn.length;f<fl;f++)self.on(name,fn[f])},self.one=function(name,fn){function proxy(){fn.apply(self,arguments),self.off(name,proxy)}self.on(name,proxy)},self.off=function(name,fn){if(fn instanceof Array)for(var f=0,fl=fn.length;f<fl;f++)self.off(name,fn[f]);else for(var names=name.split(" "),n=0,nl=names.length;n<nl;n++)if(callbacks[names[n]]=callbacks[names[n]]||[],
null==fn)callbacks[names[n]].length=0;else{var index=callbackIndex(names[n],fn);index!==-1&&callbacks[names[n]].splice(index,1)}},self.getCurrentPos=function(fn){"function"==typeof fn&&fn.call(self,pos.cur)},self.destroy=function(){return $doc.add($scrollSource).add($handle).add($sb).add($pb).add($forwardButton).add($backwardButton).add($prevButton).add($nextButton).add($prevPageButton).add($nextPageButton).unbind("."+namespace),$prevButton.add($nextButton).add($prevPageButton).add($nextPageButton).removeClass(o.disabledClass),$items&&$items.eq(rel.activeItem).removeClass(o.activeClass),$pb.empty(),parallax||($frame.unbind("."+namespace),frameStyles.restore(),
slideeStyles.restore(),sbStyles.restore(),handleStyles.restore(),$.removeData(frame,namespace)),items.length=pages.length=0,last={},self.initialized=0,self},self.init=function(){if(!self.initialized){self.on(callbackMap);var holderProps=["overflow","position"],movableProps=["position","webkitTransform","msTransform","transform","left","top","width","height"];frameStyles.save.apply(frameStyles,holderProps),sbStyles.save.apply(sbStyles,holderProps),slideeStyles.save.apply(slideeStyles,movableProps),handleStyles.save.apply(handleStyles,movableProps);var $movables=$handle;return parallax||($movables=$movables.add($slidee),$frame.css("overflow","hidden"),transform||"static"!==$frame.css("position")||$frame.css("position","relative")),
transform?gpuAcceleration&&$movables.css(transform,gpuAcceleration):("static"===$sb.css("position")&&$sb.css("position","relative"),$movables.css({position:"absolute"})),o.forward&&$forwardButton.on(mouseDownEvent,buttonsHandler),o.backward&&$backwardButton.on(mouseDownEvent,buttonsHandler),o.prev&&$prevButton.on(clickEvent,buttonsHandler),o.next&&$nextButton.on(clickEvent,buttonsHandler),o.prevPage&&$prevPageButton.on(clickEvent,buttonsHandler),o.nextPage&&$nextPageButton.on(clickEvent,buttonsHandler),$scrollSource.on(wheelEvent,scrollHandler),$sb[0]&&$sb.on(clickEvent,scrollbarHandler),itemNav&&o.activateOn&&$frame.on(o.activateOn+"."+namespace,"*",activateHandler),
$pb[0]&&o.activatePageOn&&$pb.on(o.activatePageOn+"."+namespace,"*",activatePageHandler),$dragSource.on(dragInitEvents,{source:"slidee"},dragInit),$handle&&$handle.on(dragInitEvents,{source:"handle"},dragInit),$doc.bind("keydown."+namespace,keyboardHandler),parallax||($frame.on("mouseenter."+namespace+" mouseleave."+namespace,pauseOnHoverHandler),$frame.on("scroll."+namespace,resetScroll)),self.initialized=1,load(),o.cycleBy&&!parallax&&self[o.startPaused?"pause":"resume"](),self}}}function type(value){return null==value?String(value):"object"==typeof value||"function"==typeof value?Object.prototype.toString.call(value).match(/\s([a-z]+)/i)[1].toLowerCase()||"object":typeof value;
}function stopDefault(event,noBubbles){event.preventDefault(),noBubbles&&event.stopPropagation()}function disableOneEvent(event){stopDefault(event,1),$(this).off(event.type,disableOneEvent)}function resetScroll(){this.scrollLeft=0,this.scrollTop=0}function isNumber(value){return!isNaN(parseFloat(value))&&isFinite(value)}function getPx($item,property){return 0|round(String($item.css(property)).replace(/[^\-0-9.]/g,""))}function within(number,min,max){return number<min?min:number>max?max:number}function StyleRestorer(element){var self={};return self.style={},self.save=function(){if(element){for(var i=0;i<arguments.length;i++)self.style[arguments[i]]=element.style[arguments[i]];
return self}},self.restore=function(){if(element){for(var prop in self.style)self.style.hasOwnProperty(prop)&&(element.style[prop]=self.style[prop]);return self}},self}var transform,gpuAcceleration,time,pluginName="sly",className="Sly",namespace=pluginName,cAF=w.cancelAnimationFrame||w.cancelRequestAnimationFrame,rAF=w.requestAnimationFrame,$doc=$(document),dragInitEvents="touchstart."+namespace+" mousedown."+namespace,dragMouseEvents="mousemove."+namespace+" mouseup."+namespace,dragTouchEvents="touchmove."+namespace+" touchend."+namespace,wheelEvent=(document.implementation.hasFeature("Event.wheel","3.0")?"wheel.":"mousewheel.")+namespace,clickEvent="click."+namespace,mouseDownEvent="mousedown."+namespace,interactiveElements=["INPUT","SELECT","BUTTON","TEXTAREA"],tmpArray=[],abs=Math.abs,sqrt=Math.sqrt,pow=Math.pow,round=Math.round,max=Math.max,min=Math.min,lastGlobalWheel=0;
$doc.on(wheelEvent,function(){lastGlobalWheel=+new Date}),function(w){function fallback(fn){var curr=(new Date).getTime(),ms=Math.max(0,16-(curr-prev)),req=setTimeout(fn,ms);return prev=curr,req}rAF=w.requestAnimationFrame||w.webkitRequestAnimationFrame||fallback;var prev=(new Date).getTime(),cancel=w.cancelAnimationFrame||w.webkitCancelAnimationFrame||w.clearTimeout;cAF=function(id){cancel.call(w,id)}}(window),function(){function testProp(prop){for(var p=0,pl=prefixes.length;p<pl;p++){var prefixedProp=prefixes[p]?prefixes[p]+prop.charAt(0).toUpperCase()+prop.slice(1):prop;if(null!=el.style[prefixedProp])return prefixedProp}}var prefixes=["","webkit","moz","ms","o"],el=document.createElement("div");
transform=testProp("transform"),gpuAcceleration=testProp("perspective")?"translateZ(0) ":""}(),w[className]=Sly,$.fn[pluginName]=function(options,callbackMap){var method,methodArgs;return $.isPlainObject(options)||("string"!==type(options)&&options!==!1||(method=options===!1?"destroy":options,methodArgs=Array.prototype.slice.call(arguments,1)),options={}),this.each(function(i,element){var plugin=$.data(element,namespace);plugin||method?plugin&&method&&plugin[method]&&plugin[method].apply(plugin,methodArgs):plugin=$.data(element,namespace,new Sly(element,options,callbackMap).init())})},Sly.defaults={horizontal:!1,itemNav:null,itemSelector:null,smart:!1,activateOn:null,
activateMiddle:!1,scrollSource:null,scrollBy:0,scrollHijack:300,scrollTrap:!1,dragSource:null,mouseDragging:!1,touchDragging:!1,releaseSwing:!1,swingSpeed:.2,elasticBounds:!1,interactive:null,scrollBar:null,dragHandle:!1,dynamicHandle:!1,minHandleSize:50,clickBar:!1,syncSpeed:.5,pagesBar:null,activatePageOn:null,pageBuilder:function(index){return"<li>"+(index+1)+"</li>"},forward:null,backward:null,prev:null,next:null,prevPage:null,nextPage:null,cycleBy:null,cycleInterval:5e3,pauseOnHover:!1,startPaused:!1,moveBy:300,speed:0,easing:"swing",startAt:0,keyboardNavBy:null,draggedClass:"dragged",activeClass:"active",disabledClass:"disabled"}}(jQuery,window)}),define("normalize",[],function(){
function convertURIBase(uri,fromBase,toBase){if(uri.match(absUrlRegEx)||uri.match(protocolRegEx))return uri;uri=removeDoubleSlashes(uri);var toBaseProtocol=toBase.match(protocolRegEx),fromBaseProtocol=fromBase.match(protocolRegEx);return!fromBaseProtocol||toBaseProtocol&&toBaseProtocol[1]==fromBaseProtocol[1]&&toBaseProtocol[2]==fromBaseProtocol[2]?relativeURI(absoluteURI(uri,fromBase),toBase):absoluteURI(uri,fromBase)}function absoluteURI(uri,base){if("./"==uri.substr(0,2)&&(uri=uri.substr(2)),uri.match(absUrlRegEx)||uri.match(protocolRegEx))return uri;var baseParts=base.split("/"),uriParts=uri.split("/");for(baseParts.pop();curPart=uriParts.shift();)".."==curPart?baseParts.pop():baseParts.push(curPart);
...
return".css"==name.substr(name.length-4,4)&&(name=name.substr(0,name.length-4)),normalize(name)},cssAPI.load=function(cssId,req,load,config){(useImportLoad?importLoad:linkLoad)(req.toUrl(cssId+".css"),load)},cssAPI}),define("css!parccTei/pciCreator/ims/multiTabbedExhibit/runtime/css/multiTabbedExhibit",[],function(){}),define("multiTabbedExhibit/runtime/multiTabbedExhibit",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/handlebars","taoQtiItem/portableLib/OAT/waitForMedia","parccTei/pciCreator/ims/multiTabbedExhibit/runtime/lib/sly","css!parccTei/pciCreator/ims/multiTabbedExhibit/runtime/css/multiTabbedExhibit"],function($,qtiCustomInteractionContext,event,_,handlebars){
"use strict";function initPaging($frameContainer){function init(){pages=[],$frame.sly("getCurrentPos",function(currentPos){$pages.each(function(){var $page=$(this),pos=$page.position(),h=$page.outerHeight();pages.push({top:Math.round(currentPos+pos.top),middle:Math.round(currentPos+pos.top+h/2),bottom:Math.round(currentPos+pos.top+h),$dom:$page,id:$page.data("page-id"),index:pages.length}),$page.find(".page-number").html(pages.length)}),$counterTotal.html(pages.length),moveCallback(currentPos)})}function setCurrent(num){var page;current=num,page=pages[current],$current=page.$dom,$counterCurrent.html(num+1),0===num?$previous.addClass("disabled"):$previous.removeClass("disabled"),
num===pages.length-1?$next.addClass("disabled"):$next.removeClass("disabled")}function scrollTo(num){var page=pages[num];scrolling=!0,$frame.sly("slideTo",page.top,!1),setCurrent(num)}function moveCallback(newPos){var threshold=newPos+frameHalfHeight+20,currentPage=0;for(var i in pages)if(i=parseInt(i),threshold>pages[i].top&&(!pages[i+1]||threshold<pages[i+1].top)){currentPage=i;break}setCurrent(currentPage)}var $current,$frame=$frameContainer.children(".frame"),frameHalfHeight=Math.round($frame.height()/2),$pager=$frameContainer.children(".passage-pager"),$previous=$pager.find(".passage-pager-previous button"),$next=$pager.find(".passage-pager-next button"),$counterCurrent=$frameContainer.find(".counter-current"),$counterTotal=$frameContainer.find(".counter-total"),$pages=$frameContainer.find(".page"),pages=[],current=0,scrolling=!1;
init(),$frame.sly("on","move",_.throttle(function(){scrolling?scrolling=!1:moveCallback(this.pos.cur)},600)),$frame.sly("on","load",_.throttle(init,600)),$previous.on("click",function(){!$previous.hasClass("disabled")&&current>0&&scrollTo(current-1)}),$next.on("click",function(){!$next.hasClass("disabled")&&current<pages.length-1&&scrollTo(current+1)});var pagingApi={getMoveCallback:moveCallback,reload:init,getActive:function(){if($current)return $current},setActive:function(pageId){var page=_.find(pages,{id:pageId});page&&scrollTo(page.index)}};return $frameContainer.data("paging-api",pagingApi),pagingApi}function initScrolling(pci,$frameContainer){var $frame=$frameContainer.children(".frame"),$scrollbar=$frameContainer.children(".scrollbar");
$frame.waitForMedia(function(){$frame.sly({scrollBar:$scrollbar,scrollBy:20,scrollTrap:!0,dynamicHandle:!0,clickBar:!0,dragHandle:!0,mouseDragging:!1})}),$frame.closest(".passage").on("keyup",function(e){return 40===e.keyCode&&$frame.sly("nextPage"),38===e.keyCode&&$frame.sly("prevPage"),!1}),$(window).on("resize.multiTabbedExhibit."+pci.id,function(){$frame.sly("reload")}),pci.on("resize",function(){$frame.sly("reload")})}function renderTemplate(pci,tplName,data){var source=pci.$dom.find(".templates ."+tplName).html(),template=handlebars.compile(source);return template(data||{})}function initScrollingPassageMarkup(pci,$passage){var passageContent=renderTemplate(pci,"simple",{
content:$passage.html()});$passage.html(renderTemplate(pci,"scrolling",{content:passageContent}))}function initPaggingPassageMarkup(pci,$passage){var tplData={pages:[]};$passage.find(".page").each(function(){var $page=$(this);tplData.pages.push({content:$page.html(),id:$page.data("page-id")})});var passageContent=renderTemplate(pci,"pages",tplData);$passage.html(renderTemplate(pci,"scrolling",{content:passageContent})),$passage.append(renderTemplate(pci,"pager"))}function initPassages(pci){pci.$dom.find(".passage-scrolling").each(function(){var $passage=$(this);initScrollingPassageMarkup(pci,$passage),initScrolling(pci,$passage)}),pci.$dom.find(".passage-paging").each(function(){
var $passage=$(this);initPaggingPassageMarkup(pci,$passage),initScrolling(pci,$passage),initPaging($passage)})}function initTabbingMarkup(pci){var tplData={passages:[]},$tabContainer=pci.$dom.children(".passages").addClass("passages-tabs"),$passages=$tabContainer.children(".passage").addClass("passage-tab");$passages.each(function(){var $passage=$(this);tplData.passages.push({title:$passage.attr("title"),active:$passage.hasClass("active"),id:$passage.data("passage-id")}),$passage.removeAttr("title")}),$tabContainer.children(".passages-tab-navigation").remove(),$tabContainer.prepend(renderTemplate(pci,"tab-navigation",tplData))}function initTabbing(pci){function activateTab($a){
var $li=$a.parent(),$tab=$a.data("passage-tab");$tab&&$tab.length&&($tab.show().siblings(".passage").hide(),$li.addClass("passages-tab-active").siblings("li").removeClass("passages-tab-active"),$tab.find(".frame").sly("reload"),$active=$a,$tab.trigger("activate.tab",[$tab.data("passage-id")]))}initTabbingMarkup(pci);var $tabContainer=pci.$dom.find(".passages-tabs"),$tabs=$tabContainer.children(".passage"),$nav=$tabContainer.children(".passages-tab-navigation"),i=0,$active=$nav.find("li:first-child a");$tabs.hide(),$nav.children("li").each(function(){var $li=$(this),$a=$li.children("a");if(i<$tabs.length){var $tab=$($tabs[i]);$a.data("passage-tab",$tab),$li.hasClass("passages-tab-active")&&($active=$a);
}else $a.hide();i++}),$nav.on("click","a",function(e){e.preventDefault(),activateTab($(this))}),activateTab($active),$tabContainer.on("activate.tab",function(e,id){pci.trigger("activate",[id])});var tabbingApi={activate:function(passageId){var $trigger=$nav.find("a[data-passage-id="+passageId+"]");activateTab($trigger)},getActive:function(){var $passage=$active.data("passage-tab");if($passage.length)return $passage}};return $tabContainer.attr("data-stuff",!0),$tabContainer.data("tabbing-api",tabbingApi),tabbingApi}function init(pci){initPassages(pci);var tabbed=pci.config.tabbed;tabbed&&"false"!==tabbed&&initTabbing(pci)}var _typeIdentifier="multiTabbedExhibit",multiTabbedExhibit={
typeIdentifier:_typeIdentifier,getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties,config.assetManager),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){return{base:{boolean:!0}}},destroy:function(){this.$dom.off().empty(),$(window).off("resize.multiTabbedExhibit."+this.id)},getTypeIdentifier:function(){return _typeIdentifier},initialize:function(id,dom,config){var self=this;this.id=id,this.$dom=$(dom),
this.config=config||{},event.addEventMgr(this),init(this),this.on("passagechange",function(markup,tabbed,state){var $newMarkup=$(markup);self.config.tabbed=tabbed,self.$dom.children(".passages").replaceWith($newMarkup.children(".passages")),init(self),state&&self.setSerializedState(state),self.trigger("passagereload",[state])})},setResponse:function(response){},resetResponse:function(){},setSerializedState:function(state){if(state&&state.passage){var $tabs=this.$dom.find(".passages-tabs");if($tabs.length){var tabApi=$tabs.data("tabbing-api");if(tabApi.activate(state.passage),state.page){var $passage=tabApi.getActive();if($passage&&$passage.hasClass("passage-paging")){
var pagingApi=$passage.data("paging-api");pagingApi.setActive(state.page)}}}}},getSerializedState:function(){var tabApi,pagingApi,$passage,$page,state={},$tabs=this.$dom.find(".passages-tabs");return $tabs.length&&(tabApi=$tabs.data("tabbing-api"),$passage=tabApi.getActive(),$passage&&(state.passage=$passage.data("passage-id"),$passage.hasClass("passage-paging")&&(pagingApi=$passage.data("paging-api"),$page=pagingApi.getActive(),$page&&(state.page=$page.data("page-id"))))),state}};qtiCustomInteractionContext.register(multiTabbedExhibit)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),
s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}('/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\n\n.multiTabbedExhibit {\n    width: 100%;\n    margin: 20px 0;\n    padding : 5px;\n}\n\n.multiTabbedExhibit .templates {display:none;}\n\n.multiTabbedExhibit .passage {position:relative; min-height: 100px;}\n.multiTabbedExhibit .passage240 .frame, .multiTabbedExhibit .passage240 .scrollbar {height:240px;}\n.multiTabbedExhibit .passage440 .frame, .multiTabbedExhibit .passage440 .scrollbar  {height:440px;}\n.multiTabbedExhibit .passage540 .frame, .multiTabbedExhibit .passage540 .scrollbar  {height:540px;}\n\n.multiTabbedExhibit .frame { padding: 0; width: 100%;}\n.frame .slidee { margin: 0;}\n\n.multiTabbedExhibit .scrollbar { width: 10px; position:absolute; right:-7px; top:0; }\n.multiTabbedExhibit .scrollbar .handle {\n    width: 100%;\n    height: 80px; /* overriden if dynamicHandle: 1 */\n    cursor:pointer;\n}\n.multiTabbedExhibit .scrollbar .handle .cursor{\n    background: #266D9C;\n    width:7px;\n    height:100%;\n    right:0px;\n    -webkit-border-radius: 5px;\n    -moz-border-radius: 5px;\n    border-radius: 5px;\n}\n.multiTabbedExhibit .passage-paging, .multiTabbedExhibit .passage-scrolling {\n    position:relative;\n}\n\n.multiTabbedExhibit .passage {margin : 10px 0;}\n\n.multiTabbedExhibit .passage-paging .frame {background:#EFEFEF;}\n.multiTabbedExhibit .passage-paging .frame .pages { margin: 0; padding: 10px; list-style: none; border:#CCC;}\n.multiTabbedExhibit .passage-paging .frame .pages .page {margin: 16px 10px; padding: 10px; background:white;}\n.multiTabbedExhibit .passage-paging .frame .pages li.page.shadowed {\n    -moz-box-shadow: 0px 0px 5px 2px #aaa;\n    -webkit-box-shadow: 0px 0px 5px 2px #aaa;\n    -o-box-shadow: 0px 0px 5px 2px #aaa;\n    box-shadow: 0px 0px 5px 2px #aaa;\n    filter:progid:DXImageTransform.Microsoft.Shadow(color=#aaa, Direction=NaN, Strength=5);\n}\n.multiTabbedExhibit .passage-paging .page-header, .multiTabbedExhibit .passage-paging .page-footer {position:relative; min-height:2rem;}\n.multiTabbedExhibit .passage-paging .page-header {margin-bottom: 12px;}\n.multiTabbedExhibit .passage-paging .page-header .page-number {position:absolute;right:0px;font-weight: bold; font-size:1.8rem;}\n.multiTabbedExhibit .passage-paging .page-content {position: relative; padding:0 20px; min-height:100px;}\n\n.multiTabbedExhibit .passage-paging .passage-pager {position:relative; border:solid 0 #CCC; border-top-width: 1px;}\n.multiTabbedExhibit .passage-paging .passage-pager:after {clear:both; content:""; display:table;}\n.multiTabbedExhibit .passage-paging .passage-pager button {width:96px;}\n.multiTabbedExhibit .passage-paging .passage-pager .passage-pager-previous {position:absolute; top:3px; left:3px;}\n.multiTabbedExhibit .passage-paging .passage-pager .passage-pager-next {position:absolute; top:3px; right:3px;}\n.multiTabbedExhibit .passage-paging .passage-pager .passage-pager-counter {text-align:center; padding:5px 6px; font-weight: bold;font-size:1.5rem;}\n.multiTabbedExhibit .passage-paging .passage-pager .passage-pager-counter span {margin:0 3px;}\n.multiTabbedExhibit .passage-paging .disabled {cursor:default !important;}\n\n.multiTabbedExhibit .passage-scrolling, .multiTabbedExhibit .passage-paging {border:solid 1px #CCC;}\n.multiTabbedExhibit .passage-scrolling .passage-content {position: relative; padding:1rem;}\n\n/*tab style*/\n.multiTabbedExhibit ul.passages-tab-navigation {\n    box-sizing: border-box;\n    width:100%;\n    line-height: 20px;\n    padding:0;\n    margin:0;\n    list-style: none;\n    border-bottom:1px solid #DDD;\n}\n.multiTabbedExhibit ul.passages-tab-navigation:after, .multiTabbedExhibit ul.passages-tab-navigation:before{ content:""; display:table;}\n.multiTabbedExhibit ul.passages-tab-navigation:after {clear:both;}\n\n.multiTabbedExhibit ul.passages-tab-navigation li {\n    margin-bottom:-1px;\n    float:left;\n    position: relative;\n}\n.multiTabbedExhibit ul.passages-tab-navigation li a {\n    display: block;\n    padding: 8px 12px;\n    border-width: 1px 1px 0px;\n    border-style: solid;\n    border-color: transparent;\n    -moz-border-top-colors: none;\n    -moz-border-right-colors: none;\n    -moz-border-bottom-colors: none;\n    -moz-border-left-colors: none;\n    border-image: none;\n    color: #1D8ACB;\n    text-decoration: none;\n    border-radius: 4px 4px 0px 0px;\n    text-shadow: 0px 1px 0px white;\n    cursor:pointer;\n    outline: 0px none;\n}\n.multiTabbedExhibit ul.passages-tab-navigation li a:focus {\n    border-color: #DDD;\n    border-style: dashed;\n    color: #0B5F90 !important;\n}\n.multiTabbedExhibit ul.passages-tab-navigation li:not(.passages-tab-active) a:hover {\n    border-color: #DDD;\n    background: #F1F1F1;\n    color: #0B5F90;\n    outline: 0px none;\n    margin-bottom: 1px;\n    padding-bottom: 7px;\n}\n\n.multiTabbedExhibit ul.passages-tab-navigation > li:nth-child(n+2) > a {margin-left: 3px;}\n\n.multiTabbedExhibit ul.passages-tab-navigation li.passages-tab-active a{\n    border-color: #DDD #DDD transparent;\n    color: #666;\n    background: white;\n    cursor:default;\n}\n'),
define(["multiTabbedExhibit/runtime/multiTabbedExhibit"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=multiTabbedExhibit.min.js.map